...
export function SuppliersSearch() {
  ...
  const go = () => {
    ...
    router.push(`/suppliers${p.toString() ? `?${p}` : ""}`);
  };
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); start(go); }} className="mb-5 flex gap-2">
      <div className="relative max-w-sm flex-1">
        <input
          className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-9 text-sm text-slate-900 shadow-[var(--shadow-soft)] outline-none transition focus:border-navy-600 focus:ring-2 focus:ring-navy-600/15"
      ...
      {q && (
        <button type="button" onClick={() => { setQ(""); start(() => router.push("/suppliers")); }} className="rounded-lg px-3 py-2 text-sm font-medium text-slate-500 hover:text-slate-800">Clear</button>
      )}
    </form>
  );
}
